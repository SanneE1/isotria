...
ger$germ<-ger$n_rec/ger$n_fruit_t0

germin<- ger[complete.cases(ger$germ),]
View (germin)
mean_germination<-mean(germin$germ)
germinatin<-mean_germination

#model for germination
germ_mod<-lmer(germ ~ Site + (1 | year_t1), data= germin)
germ_mod
...
fec<-function(x,y, pars, h, site){
  xb <- x_range(x, pars)
 return( flx(xb,pars, site) * fln(xb,pars, site) * pars$fruiting * nrxy(y,pars, h) * pars$ge_b0)
}
