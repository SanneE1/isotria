...
dat_stan <- list(
  n       = nrow(flow_df),
  n_time  = nrow(snow_an),
  n_year  = as.factor(flow_df$year_t1) %>% unique %>% length,
  n_lag   = ncol(snow_an),
  y       = flow_df$fruit_t1,
  size_t0 = flow_df$log_size_t1,
  clim    = t(snow_an),
  mean_c  = snow_an[,'V3'],
  year_i  = as.factor(flow_df$year_t1) %>% as.numeric
)
...
tiff('results/demography/flow/snow_depth_weights.tiff',
...
tiff('results/demography/flow/ppt.tiff',
     unit="in", width=4.5, height=4.5, res=600,compression="lzw")
...
tiff('results/demography/flow/airt.tiff',
     unit="in", width=4.5, height=4.5, res=600,compression="lzw")
...
save.image( 'results/models/flow_mod_sel.Rdata')
